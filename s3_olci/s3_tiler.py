...
def convert_geoloc_vrt(data_dir):
...
def convert_nc_geotiff(data_dir, selected_bands):
    ...
    fname = (data_dir / "S3_data.tif").as_posix()
    output_ds = driver.Create(fname,
...
def georeference_tif(n_bands, xsize, ysize, output_file, dest_dir):
    ...
    lon_file = (dest_dir / "lon.tif").as_posix()
    lat_file = (dest_dir / "lat.tif").as_posix()
    ...
    # save VRT file    
    (dest_dir/"Sentinel_data.vrt").write_text(vrt_main_tmpl)
