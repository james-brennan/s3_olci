...
if __name__ =="__main__":
    ...
    #sen3file = sys.argv[1]
    files  = np.genfromtxt("s3_files.txt", dtype=str)
    
    
    SWATHS = []   
    for sen3file in files:
        ...
        SWATHS.append(olci_swath)
    SWATHS = np.array(SWATHS)
    
    # get poly for the tile
    ...
    idx = np.where(tiles==OUR_TILE)[0][0]
    TILE_POLY = MODIS_TILES[idx]



    # ---------- Find intersecting swaths for this tile  ----------------------
    intersects = [TILE_POLY.intersects(img) for img in SWATHS]
    idx = np.where(intersects)
    which_imgs = files[idx]

    
    for sen3file in which_imgs:
        sen3filep = Path(sen3file)
